  ...
  if (!must_make)
    {
      ...
      //printf ("No need to remake target '%s', %d\n", file->name, file->is_target);
      if (!is_incremental && file->is_target) 
        {
	  is_incremental = 1;
	  printf("Some objects are already created...\n");
        }
      notice_finished_file (file);
